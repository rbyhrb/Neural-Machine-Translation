...
class Lang:
	def __init__(self, file_name, vocab_size, lang_name):
		cmd = '--input=' + file_name + ' --model_prefix=' + lang_name +' --vocab_size=' + str(vocab_size) + ' --pad_id=2 --unk_id=3 --bos_id=0 --eos_id=1 --pad_piece=[PAD] --unk_piece=[UNK] --bos_piece=[BOS] --eos_piece=[EOS] --normalization_rule_name=nmt_nfkc_cf --model_type=bpe'
		spm.SentencePieceTrainer.train(cmd)
		self.sp = spm.SentencePieceProcessor()
		self.sp.load(lang_name+'.model')
...
def readLangs(lang1, lang2, test_lang1, test_lang2, lang_name1, lang_name2, vocab_size1, vocab_size2, reverse=False):
	print("Reading lines...")
	...
	pairs = []
	pairs = [[lines_lang1[i].lower(), lines_lang2[i].lower()] for i in range(len(lines_lang1))]
	...
	if reverse:
		...
		input_lang = Lang(lang2, vocab_size2, lang_name2)
		output_lang = Lang(lang1, vocab_size1, lang_name1)
	else:
		input_lang = Lang(lang1, vocab_size1, lang_name1)
		output_lang = Lang(lang2, vocab_size2, lang_name2)
